...
`phi` dates back to 2016 with it's original implementation as one of the first routers to adopt the newly introduced
context.Context api to the stdlib -- set out to design a router that is faster, more modular and simpler than anything
else out there -- while not introducing any custom handler types or dependencies. Today, `phi` still has zero dependencies,
and in many ways is future proofed from changes, given it's minimal nature. Between versions, phi's iterations have been very
incremental, with the arphitecture and api being the same today as it was originally designed in 2016. For this reason it 
makes phi a pretty easy project to maintain, as well thanks to the many amazing community contributions over the years
to who all help make phi better (total of 86 contributors to date -- thanks all!).
...
and always improving the code wherever possible -- and as of today the `phi` package is just 1082 lines of code (not counting
middlewares, which are all optional). As well, I don't have the exact metrics, but from my analysis and email exchanges from
companies and developers, phi is used by thousands of projects around the world -- thank you all as there is no better form of
joy for me than to have art I had started be helpful and enjoyed by others. And of course I use phi in all of my own projects too :)
...
is adopted everywhere, it's time for phi to get with the times. Luckily, I've discovered a path forward that will make me happy,
...
`go get github.com/go-phi/phi` or `go get github.com/go-phi/phi@latest` and they will get the latest version which contains
go.mod support, which is v1.5.0+. `phi` will not change very much over the years, just like it hasn't changed much from 4 years ago.
...
For existing projects who want to upgrade to the latest go.mod version, run: `go get -u github.com/go-phi/phi@v1.5.0`,
which will get you on the go.mod version line (as Go's mod cache may still remember v4.x). Brand new systems can run
`go get -u github.com/go-phi/phi` or `go get -u github.com/go-phi/phi@latest` to install phi, which will install v1.5.0+
...
- phi v4 requires Go 1.10.3+ (or Go 1.9.7+) - we have deprecated support for Go 1.7 and 1.8
...
- middleware: new `ContentCharset` to check matphing charsets. Thank you
...
- New phi.RegisterMethod(method) to add support for custom HTTP methods, see _examples/custom-method for usage
- Deprecated LINK and UNLINK methods from the default list, please use `phi.RegisterMethod("LINK")` and `phi.RegisterMethod("UNLINK")` in an `init()` function
...
- Major update to phi library with many exciting updates, but also some *breaking changes*
- URL parameter syntax changed from `/:id` to `/{id}` for even more flexible routing, such as
  `/articles/{month}-{day}-{year}-{slug}`, `/articles/{id}`, and `/articles/{id}.{ext}` on the
  same router
- Support for regexp for routing patterns, in the form of `/{paramKey:regExp}` for example:
  `r.Get("/articles/{name:[a-z]+}", h)` and `phi.URLParam(r, "name")`
- Add `Method` and `MethodFunc` to `phi.Router` to allow routing definitions such as
...
- Minor improvements and update to the phi core library
- Introduced a brand new `phi/render` sub-package to complete the story of building
  APIs to offer a pattern for managing well-defined request / response payloads. Please
  check out the updated `_examples/rest` example for how it works.
- Added `MethodNotAllowed(h http.HandlerFunc)` to phi.Router interface
...
- After many months of v2 being in an RC state with many companies and users running it in
  ...
  announce v2.0.0 of phi.


## v2.0.0-rc1 (2016-07-26)

- Huge update! phi v2 is a large refactor targetting Go 1.7+. As of Go 1.7, the popular
  community `"net/context"` package has been included in the standard library as `"context"` and
  utilized by `"net/http"` and `http.Request` to managing deadlines, cancelation signals and other
  request-scoped values. We're very excited about the new context addition and are proud to
  introduce phi v2, a minimal and powerful routing package for building large HTTP services,
  with zero external dependencies. Chi focuses on idiomatic design and encourages the use of 
  stdlib HTTP handlers and middlwares.
- phi v2 deprecates its `phi.Handler` interface and requires `http.Handler` or `http.HandlerFunc`
- phi v2 stores URL routing parameters and patterns in the standard request context: `r.Context()`
- phi v2 lower-level routing context is accessible by `phi.RouteContext(r.Context()) *phi.Context`,
  which provides direct access to URL routing parameters, the routing path and the matphing
  routing patterns.
- Users upgrading from phi v1 to v2, need to:
  1. Update the old phi.Handler signature, `func(ctx context.Context, w http.ResponseWriter, r *http.Request)` to
     the standard http.Handler: `func(w http.ResponseWriter, r *http.Request)`
  2. Use `phi.URLParam(r *http.Request, paramKey string) string`
